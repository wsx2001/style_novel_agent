...
import { generationUrls } from '@/api/generations'
...
export function GenerationPanel({
  ...
  const run = async (type: 'continue' | 'rewrite') => {
    ...
    const body = type === 'continue'
      ? { ...base, prompt: prompt || undefined, view: view || undefined }
      : {
          ...base,
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <label className="text-xs text-slate-500">
            ...
            <input
              ...
              step={0.1}
              value={temperature}
              onChange={(e) => setTemperature(Math.min(2, Math.max(0, Number(e.target.value) || 0.8)))}
              className="mt-1 w-full rounded-md border border-border bg-background px-2 py-1 text-sm focus:border-primary focus:outline-none"
            />
          </label>
        </div>
